...
export default class ActionPanel extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.actionButtonList = Array.from(document.querySelectorAll('action-button'));
    ...
    this.drawButton = document.querySelector(`action-button[action-type='${EActionType.DRAW}']`);
    this.clockwiseButton = document.querySelector(`action-button[action-type='${EActionType.CHANGE_CLOCKWISE}']`);
    this.clearButton = document.querySelector(`action-button[action-type='${EActionType.CLEAR}']`);
    ...
    switch (name) {
      ...
      case 'state': {
        ...
        if (newValue === EActionType.CLEAR) {
          ...
          if (this.firstPointElement) {
            this.firstPointElement.innerHTML = '';
          }

          if (this.secondPointElement) {
            this.secondPointElement.innerHTML = '';
          }
